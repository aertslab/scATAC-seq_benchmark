...
def main():
    if len(sys.argv) != 2:
        print >> sys.stderr, '\nUsage: ctx-check-availability check-availability.ini\n\n' + \
                             'Example configfile:\n-------------------\n\n' + \
                             '[dbserver]\n' + \
                             'servername=localhost\n' + \
                             'port=3306\n' + \
                             'username=icistargetdaemon\n' + \
                             'password=<base64_encoded_password>\n' + \
                             'database=icistarget_daemon\n' + \
                             '[compserver]\n' + \
                             'email_subject=Problem with i-cisTarget daemon on ' + platform.node() + ' ...\n' + \
                             'webserver_email_address=noreply@example.com\n' + \
                             'recipient_email_addresses=yourname@example.com\n' + \
                             'max_waiting_time_in_min=20\n' + \
                             'max_jobs_in_queue=5\n' + \
                             'daemon_pid_file=<path_to_ctx-daemon.pid>\n'
        sys.exit(1)
    ...
    if os.path.exists(daemon_pid_file) and os.path.isfile(daemon_pid_file):
        ...
        if not os.path.exists('/proc/' + daemon_pid):
            send_mail(cfg,
                      "The i-cisTarget daemon with PID={0:s} is not running anymore on {1:s}.".format(daemon_pid,
                                                                                                      platform.node()))
            print datetime.now()
            print "The i-cisTarget daemon with PID={0:s} is not running anymore on {1:s}.".format(daemon_pid,
                                                                                                  platform.node())
        elif os.path.exists(daemon_pid_cmdline_file):
            with open(daemon_pid_cmdline_file, 'r') as fh:
                cmdline = fh.read()

            # Check the PID is indeed from the ctx-daemon.
            if 'ctx-daemon' in cmdline:
                ...
                if jobs_in_queue > max_jobs_in_queue or waiting_time >= max_waiting_time_in_min:
                    send_mail(cfg,
                              "There are {0:d} jobs in the job queue of the i-cisTarget daemon on {1:s}\nand the waiting time for a requested job exceeds {2:d} minutes.".format(
                ...
                send_mail(cfg,
                          "PID {0:s} is in use by another process.\nDelete the PID file '{1:s}' and start the i-cisTarget daemon on {2:s} again.".format(
        ...
        print datetime.now()
        print "The i-cisTarget daemon is not started on {0:s}.\nPID file '{1:s}' could not be found.".format(
