...
def create_db_connection(cfg, new=False):
    if new:
        return connect(hostname=cfg.get('dbserver', 'servername'),
                       port=cfg.getint('dbserver', 'port'),
                       username=cfg.get('dbserver', 'username'),
                       password=base64.b64decode(cfg.get('dbserver', 'password')))
    else:
        return connect(hostname=cfg.get('dbserver', 'servername'),
                       port=cfg.getint('dbserver', 'port'),
                       username=cfg.get('dbserver', 'username'),
                       password=base64.b64decode(cfg.get('dbserver', 'password')),
                       database=cfg.get('dbserver', 'database'))
